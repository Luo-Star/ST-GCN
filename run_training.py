...
train_data = np.load('second_data/30TR5/train_data.npy')
train_label = np.load('second_data/30TR5/train_label.npy')
test_data = np.load('second_data/30TR5/val_data.npy')
test_label = np.load('second_data/30TR5/val_label.npy')
...
for epoch in range(100001): # number of mini-batches
    ...
    train_data_batch = np.zeros((batch_size,1,30,116,1))
    train_label_batch = train_label[idx_batch]

    for i in range(batch_size):
        # r1 = random.randint(0, train_data.shape[2]-W)
        train_data_batch[i]  = train_data[idx_batch[i],:,:,:,:]
    ...
    if epoch % 1000 == 0:    # print every K mini-batches
        ...
        torch.save(net.state_dict(), 'output/checkpoint.pth')
